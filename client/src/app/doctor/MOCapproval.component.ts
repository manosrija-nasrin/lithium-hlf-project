...
export class MOCapprovalService {
  private apiUrl = 'http://localhost:3001/doctor/MOCapproval';  // Replace with your actual API endpoint

  constructor(private http: HttpClient) {}

  getApprovalData(doctorId: string): Observable<any> {
    const url = `${this.apiUrl}?doctorId=${doctorId}`;
    console.log(`Fetching approval data for doctor ID: ${doctorId}`); // Debug log
    return this.http.get<any>(url).pipe(
      catchError(this.handleError)
    );
  }

  sendPostRequest(slipNumber: string, doctorId: string): Observable<any> {
    const postUrl = 'http://localhost:3001/doctor/sendMOCapproval'; // Replace with your actual POST endpoint
    return this.http.post<any>(postUrl, { slipNumber, doctorId }).pipe(
      catchError(this.handleError),
      tap(() => {
...
export class MOCapprovalComponent implements OnInit {
  approvalData!: any[];
  errorMessage!: string;
  doctorId!: string;
  ...
  constructor(
    private mocapprovalService: MOCapprovalService,
  ...
  loadApprovalData(): void {
    this.loading = true;
    this.mocapprovalService.getApprovalData(this.doctorId).subscribe(
      data => {
  ...
  sendPostRequest(slipNumber: string): void {
    this.mocapprovalService.sendPostRequest(slipNumber, this.doctorId).subscribe(
      response => {
