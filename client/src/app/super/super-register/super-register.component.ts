...
@Component({
  selector: 'app-super-new',
  templateUrl: './super-register.component.html',
  styleUrls: ['./super-register.component.scss']
})

export class SuperRegisterComponent implements OnInit, OnDestroy {
  ...
  private generateUsername(registration: string, hospitalID: string): void {
    ...
    if (registration && hospitalID) {
      ...
      const username = hospitalAbbreviation + '-SUP' + registration;
  ...
  public save(): void {
    ...
    this.sub = this.superService.createSuper(this.form.value)
      .subscribe(
        x => {
          const supRegResponse = x;
          if (supRegResponse.error) {
            this.error = supRegResponse.error;
          }
          this.router.navigate(['/', 'admin', this.getAdminUsername()]);
        },
        error => {
          // Handle the error appropriately
          console.error('Error occurred while creating super:', error);
          // Optionally, you can set an error flag or message to display to the user
          this.error = 'An error occurred while saving the super details.';
        }
      );
  }
}
