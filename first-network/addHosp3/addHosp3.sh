...
function generateHosp3() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    ${CONTAINER_CLI_COMPOSE} -f ${COMPOSE_FILE_CA_BASE} -f $COMPOSE_FILE_CA_HOSP3 up -d 2>&1
  ...
  infoln "Generating CCP files for Hosp3"
  ./ccp-generate.sh
}
...
function generateHosp3Definition() {
  ...
  infoln "Generating Hosp3 organization definition"
  ...
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
    fatalln "Failed to generate Hosp3 organization definition..."
  fi
}
...
function Hosp3Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/hosp3.lithium.com/peers/peer0.hosp3.lithium.com/
  fi
...
function addHosp3 () {
  ...
  infoln "Bringing up Hosp3 peer"
  Hosp3Up

  # Create the configuration transaction needed to add
  # Hosp3 to the network
  infoln "Generating and submitting config tx to add Hosp3"
  export FABRIC_CFG_PATH=${PWD}/../../config
  . ../scripts/hosp3-scripts/updateChannelConfig.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    fatalln "ERROR !!!! Unable to create config tx"
  fi

  infoln "Joining Hosp3 peers to network"
  . ../scripts/hosp3-scripts/joinChannel.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    fatalln "ERROR !!!! Unable to join Hosp3 peers to network"
  fi
}
...
if [ "$MODE" == "up" ]; then
  infoln "Adding hosp3 to channel '${CHANNEL_NAME}' with '${CLI_TIMEOUT}' seconds and CLI delay of '${CLI_DELAY}' seconds and using database '${DATABASE}'"
  echo
elif [ "$MODE" == "down" ]; then
  EXPMODE="Stopping network"
elif [ "$MODE" == "generate" ]; then
  EXPMODE="Generating certs and organization definition for Hosp3"
else
  printHelp
  exit 1
fi
