...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.lithium.com/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.lithium.com/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.lithium.com/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.lithium.com/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.lithium.com/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.lithium.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.lithium.com-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.lithium.com-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer.lithium.com

    mkdir -p organizations/ordererOrganizations/lithium.com/ca
    mkdir -p organizations/ordererOrganizations/lithium.com/msp/cacerts
    mkdir -p organizations/ordererOrganizations/lithium.com/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/lithium.com/orderers
    mkdir -p organizations/ordererOrganizations/lithium.com/tlsca

    CERT_DIR=organizations/ordererOrganizations/lithium.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.lithium.com-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.lithium.com-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/lithium.com/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/lithium.com/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.lithium.com-cert.pem"
