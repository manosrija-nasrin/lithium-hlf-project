...
const peerName = 'peer0.hosp1.lithium.com';
const mspId = process.env.MSP_ID ?? 'hosp1MSP';

// Path to crypto materials.
const cryptoPath = path.resolve(process.env.CRYPTO_PATH ?? path.resolve(__dirname, '..', '..', '..', 'first-network', 'organizations', 'peerOrganizations', 'hosp1.lithium.com'));

// Path to user private key directory.
const keyDirectoryPath = path.resolve(process.env.KEY_DIRECTORY_PATH ?? path.resolve(__dirname, cryptoPath, 'users', 'User1@hosp1.lithium.com', 'msp', 'keystore'));

// Path to user certificate.
const certPath = path.resolve(process.env.CERT_PATH ?? path.resolve(__dirname, cryptoPath, 'users', 'User1@hosp1.lithium.com', 'msp', 'signcerts', 'cert.pem'));
